...
application = None
try:
    if BOT_TOKEN and CHAT_ID and BOT_TOKEN != "your-telegram-bot-token" and CHAT_ID != "your-telegram-chat-id":
        application = Application.builder().token(BOT_TOKEN).build()
        logger.info("Telegram bot initialized successfully")
    else:
        logger.warning("BOT_TOKEN or CHAT_ID not set or using default values, Telegram bot not initialized")
...
def upload_to_github(file_path, file_name):
    try:
        ...
        logger.error(f"Error uploading {file_name} to GitHub: {str(e)}\n{traceback.format_exc()}")
...
def download_from_github(file_name, destination_path):
    try:
        ...
        logger.error(f"Error downloading {file_name} from GitHub: {str(e)}\n{traceback.format_exc()}")
        return False
...
def init_db():
    try:
        ...
        if not c.fetchone():
            c.execute("INSERT INTO admin (id, passphrase) VALUES (?, ?)", (1, ADMIN_PASSPHRASE))
            logger.info("Created default admin passphrase")
